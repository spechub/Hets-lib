...
header {* Basic notions about monadic programs *}
...
constdefs
  ...
  "dis(p) == (do {x\<leftarrow>p; ret()}) = ret ()"
  -- {* Copyable programs *}
  ...
  "dsef(p) == cp(p) \<and> dis(p) \<and> (\<forall>q::bool T. cp(q) \<and> dis(q) \<longrightarrow> 
...
text {*
  This is Lemma 4.5 of \cite{pdlpaper} that allows us to actually discard
...
text {* 
  This is Lemma 4.22 of \cite{pdlpaper} which allows us to insert or remove
...
text {*
  This is Lemma 4.23 of \cite{pdlpaper}, asserting a weak composability of copyable programs.
...
proof -
  assume c: "cp p"
    (* have c_unf: "do {x\<leftarrow>p; y\<leftarrow>p; ret(x,y)} = do {x\<leftarrow>p; ret(x,x) *)
...
text {* Of course, @{term "dsef"} are stable under sequencing, but this
        weak form comes in handy sometimes  *}
lemma weak_dsef_seq: "dsef p \<Longrightarrow> dsef (do {x\<leftarrow>p; ret (f x)})"
proof (unfold dsef_def)
  { assume cp_p: "cp p" and dis_p: "dis p"
    assume cpa: " \<forall>q::bool T. cp q \<and> dis q \<longrightarrow> cp (do {x\<leftarrow>p; y\<leftarrow>q; ret(x,y)})"
    have p1: "cp (do {x\<leftarrow>p; ret (f x)})" by (rule weak_cp_seq)
    moreover have p2: "dis (do {x\<leftarrow>p; ret (f x)})" by (rule weak_dis_seq)
    moreover have p3: "(\<forall>q::bool T. cp q \<and> dis q \<longrightarrow> 
                            cp (do {x\<leftarrow>do {x\<leftarrow>p; ret (f x)}; y\<leftarrow>q; ret (x, y)}))"
    proof 
      fix q
      show "cp (q::bool T) \<and> dis q \<longrightarrow> cp (do {x\<leftarrow>do {x\<leftarrow>p; ret (f x)}; y\<leftarrow>q; ret (x, y)})"
      proof
	assume "cp q \<and> dis q"
	show "cp (do {x\<leftarrow>do {x\<leftarrow>p; ret (f x)}; y\<leftarrow>q; ret (x, y)})"
	proof -
	  have "cp (do {x\<leftarrow>p; y\<leftarrow>q; ret (f x, y)})" 
	  proof (rule cp_seq_ret)
	    have "cp q \<and> dis q \<longrightarrow> cp (do {x\<leftarrow>p; y\<leftarrow>q; ret(x,y)})" by (rule spec)
	    thus "cp (do {x\<leftarrow>p; y\<leftarrow>q; ret (x,y)})" ..
	  qed
	  thus ?thesis by (simp add: mon_ctr)
	qed
      qed
    qed
    ultimately have "cp (do {x\<leftarrow>p; ret (f x)}) \<and> dis (do {x\<leftarrow>p; ret (f x)}) \<and>
                     (\<forall>q::bool T. cp q \<and> dis q \<longrightarrow> 
                                   cp (do {x\<leftarrow>do {x\<leftarrow>p; ret (f x)}; y\<leftarrow>q; ret (x, y)}))"
      by blast
  }
  thus "cp p \<and> dis p \<and> (\<forall>q::bool T. cp q \<and> dis q \<longrightarrow> cp (do {x\<leftarrow>p; y\<leftarrow>q; ret (x, y)})) \<Longrightarrow>
    cp (do {x\<leftarrow>p; ret (f x)}) \<and>
    dis (do {x\<leftarrow>p; ret (f x)}) \<and>
    (\<forall>q::bool T. cp q \<and> dis q \<longrightarrow> cp (do {x\<leftarrow>do {x\<leftarrow>p; ret (f x)}; y\<leftarrow>q; ret (x, y)}))"
    by blast
qed
...
text {*
  ...
  4.24 of \cite{pdlpaper} where @{text "X"} is the respective premiss
...
text {*
  ...
  in \cite{pdlpaper}, but has not been shown.
*}
...
proof -
  ...
  from a1 have disp: "dis p" by (unfold dsef_def, blast)
  from a1 have cpp: "cp p" by (unfold dsef_def, blast)
  ...
  proof (unfold dis_def)
    ...
    also from disq have "do {x\<leftarrow>p; q x; ret ()} = do {x\<leftarrow>p; ret ()}" 
      by (simp add: dis_left)
    also from disp have "\<dots> = ret ()" by (simp add: dis_def)
    finally show "do {x\<leftarrow>?s; ret ()} = ret ()" .
  qed
  ...
  proof -
	...
	thus "?A u = ?B u" by (rule spec)
      ...
      thus ?thesis by simp
    qed
    also from cpp have "\<dots> = do {u\<leftarrow>p; x\<leftarrow>q u; y\<leftarrow>q u; ret (x,y)}"
      by (simp add: cp_arb)
    also from cpq have "\<dots> = do {u\<leftarrow>p; x\<leftarrow>q u; ret (x,x)}"
      by (simp add: cp_arb)
    also have "\<dots> = do {x\<leftarrow>?s; ret (x,x)}" by simp
    finally show ?thesis by (simp only: cp_def)
  qed
  ...
  proof
	...
	have "do {x\<leftarrow>?w; y\<leftarrow>?w; ret (x,y)} = 
	      do {u\<leftarrow>p; x\<leftarrow>q u; y\<leftarrow>qa; a\<leftarrow>ret(x,y); 
                  u'\<leftarrow>p; x'\<leftarrow>q u'; y'\<leftarrow>qa; b\<leftarrow>ret(x',y'); ret(a,b)}"
	  by (simp del: bind_assoc add: mon_ctr)
	also have "\<dots> = do {u\<leftarrow>p; x\<leftarrow>q u; y\<leftarrow>qa; u'\<leftarrow>p; x'\<leftarrow>q u'; y'\<leftarrow>qa; ret((x,y),(x',y'))}"
	  by simp
	...
	have "\<dots> = do {u\<leftarrow>p; u'\<leftarrow>p; x\<leftarrow>q u; x'\<leftarrow>q u'; y\<leftarrow>qa; y'\<leftarrow>qa; ret((x,y),(x',y'))}"
	  by (simp add: commute_bool)
	also from cpp have "\<dots> = do {u\<leftarrow>p; x\<leftarrow>q u; x'\<leftarrow>q u; y\<leftarrow>qa; y'\<leftarrow>qa; ret((x,y),(x',y'))}"
	  by (rule cp_arb)
	also from cpq have "\<dots> = do {u\<leftarrow>p; x\<leftarrow>q u; y\<leftarrow>qa; y'\<leftarrow>qa; ret((x,y),(x,y'))}"
	  by (simp add: cp_arb)
	also from cpqa have "\<dots> = do {u\<leftarrow>p; x\<leftarrow>q u; y\<leftarrow>qa; ret((x,y),(x,y))}"
	  by (simp add: cp_arb)
	also have "\<dots> = do {x\<leftarrow>do {u\<leftarrow>p; x\<leftarrow>q u; y\<leftarrow>qa; ret(x,y)}; ret(x,x)}"
	  by (simp add: mon_ctr del: bind_assoc)
	finally show ?thesis by (simp add: cp_def)
      qed
    }
    thus "cp (qa::bool T) \<and> dis qa \<longrightarrow> cp (do {x\<leftarrow>do {u\<leftarrow>p; q u}; y\<leftarrow>qa; ret (x, y)})" by blast
  qed
