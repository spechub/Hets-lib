theory LinearAlgebra_EuclideanSpaceIsNormedSpace_T
...
ML "Header.initialize
    [\"subtype_def\", \"subtype_pred_def\",
     ...
     \"right_homogeneous\", \"non_degenerate\", \"sqr_def\",
     \"sqrt_def\", \"lindep_def\", \"lindep_reflexivity\",
     \"lindep_symmetry\", \"simple_lindep_condition\",
     \"lindep_nonlindep_transitivity\", \"norm_from_inner_prod_def\",
     \"proj_def\", \"orthcomp_def\", \"orthogonal_def\",
     \"orth_symmetric\", \"lindep_orth_transitive\",
     \"orthogonal_on_zero_projection\",
     \"orthogonal_projection_theorem\",
     \"orthogonal_decomposition_theorem\",
     \"unique_orthogonal_decomposition\", \"homogen\", \"triangle\",
     \"definit\", \"nonXMinusnegative\", \"posXMinusdefinite\",
     \"multiplic\", \"triangle_1\"]"
...
XVBarXVBar__XVBarXVBar :: "Space => Real" ("(||/ _/ ||)" [10] 999)
...
X_lindep :: "Space => Space => bool" ("lindep/'(_,/ _')" [3,3] 999)
X_max :: "Real => Real => Real" ("max''/'(_,/ _')" [3,3] 999)
X_min :: "Real => Real => Real" ("min''/'(_,/ _')" [3,3] 999)
X_orth :: "Space => Space => bool" ("orth/'(_,/ _')" [3,3] 999)
X_orthcomp :: "Space => Space => Space" ("orthcomp/'(_,/ _')" [3,3] 999)
X_proj :: "Space => Space => Space" ("proj/'(_,/ _')" [3,3] 999)
...
theorem homogen :
"ALL a. ALL v. || a *_3 v || = gn_inj(abs'(a)) *'' || v ||"
...
ML "Header.record \"triangle\""

theorem definit : "ALL v. || v || = 0' = (v = 0'')"
using subtype_def subtype_pred_def Ax1_1 inf_def
      RealNonNeg_pred_def RealPos_pred_def abs_def sqr_def sqrt_def
      lindep_def norm_from_inner_prod_def proj_def orthcomp_def
      orthogonal_def
...
ML "Header.record \"definit\""

theorem nonXMinusnegative : "ALL x. gn_inj(abs'(x)) >=' 0'"
using subtype_def subtype_pred_def Ax1_1 inf_def
      RealNonNeg_pred_def RealPos_pred_def abs_def sqr_def sqrt_def
      lindep_def norm_from_inner_prod_def proj_def orthcomp_def
      orthogonal_def
...
ML "Header.record \"nonXMinusnegative\""

theorem posXMinusdefinite :
"ALL x. gn_inj(abs'(x)) = makePartial 0' = (x = 0')"
using subtype_def subtype_pred_def Ax1_1 inf_def
      RealNonNeg_pred_def RealPos_pred_def abs_def sqr_def sqrt_def
      lindep_def norm_from_inner_prod_def proj_def orthcomp_def
      orthogonal_def
...
ML "Header.record \"posXMinusdefinite\""

theorem multiplic :
"ALL x.
 ALL y.
 gn_inj(abs'(x *'' y)) =
 makePartial (gn_inj(abs'(x)) *'' gn_inj(abs'(y)))"
using subtype_def subtype_pred_def Ax1_1 inf_def
      RealNonNeg_pred_def RealPos_pred_def abs_def sqr_def sqrt_def
      lindep_def norm_from_inner_prod_def proj_def orthcomp_def
      orthogonal_def
...
ML "Header.record \"multiplic\""

theorem triangle_1 :
"ALL x.
 ALL y. gn_inj(abs'(x +' y)) <=' gn_inj(abs'(x)) +' gn_inj(abs'(y))"
using subtype_def subtype_pred_def Ax1_1 inf_def
      RealNonNeg_pred_def RealPos_pred_def abs_def sqr_def sqrt_def
      lindep_def norm_from_inner_prod_def proj_def orthcomp_def
      orthogonal_def
